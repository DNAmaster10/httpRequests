        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    plugin.getLogger().warning("Malformed URL in whitelist at line " + lineNumber);
                }
            }
        }
        catch(Exception e) {
            plugin.getLogger().severe("Failed to load whitelist: " + e);
            return false;
        }
        return true;
    }
    ...
    public static void reloadWhitelist(CommandSender sender) {
        ...
        if (loadWhitelist()) {
            if (sender instanceof Player p) {
                p.sendMessage(ChatColor.GREEN + "Successfully reloaded the whitelist!");
            }
            plugin.getLogger().info("Successfully reloaded the whitelist");
        }
        else {
            if (sender instanceof Player p) {
                p.sendMessage(ChatColor.RED + "Failed to reload the whitelist! Check server logs for more details.");
            }
    ...
    public static boolean checkWhitelist(String address) {
        try {
            ...
            if (plugin.getConfig().getBoolean("PrintRequestsToConsole")) {
                plugin.getLogger().warning("Failed to do whitelist check for a request. Possible malformed URL. URL: " + address);
            }
            return false;
        }
        return true;
